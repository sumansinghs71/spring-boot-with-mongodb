...
export class AnalysisComponent implements OnInit {
  ...
  lastThreeMonthsChartClick(event) {
    let clickedCategory = event.point.category.replace(/ /g, '');
    let vehicleType = event.point.series.name;
    this.plateDetailsService.getMonthlyVehicleDetails(vehicleType, clickedCategory.split("-")[0], clickedCategory.split("-")[1]).subscribe(data => {
  ...
  periodicalParkingChartClick(event) {
    let clickedCategory = event.point.category.replace(/ /g, '');
    let vehicleType = event.point.series.name;
    this.plateDetailsService.getPeriodicalVehicleDetails(vehicleType, clickedCategory).subscribe(data => {
      this.vehicleDurationDetails = data;
      $("#vehicleModal").modal('show');
    });
  }
  ...
  renderPeriodicSummary() {
    this.parkingSummary = new Chart({
      ...
      plotOptions: {
        ...
        series: {
          events: {
            click: this.lastThreeMonthsChartClick.bind(this)
          }
        }
      },
      legend: {
  ...
  renderPeriodicChart() {
    this.periodicChart = new Chart({
      ...
      plotOptions: {
        ...
        series: {
          events: {
            click: this.periodicalParkingChartClick.bind(this)
          }
        }
      },
      legend: {
